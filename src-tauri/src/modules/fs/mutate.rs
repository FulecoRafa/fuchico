...
mod tests {
    ...
    fn create_dir_builds_nested_chain_and_refuses_existing() {
        ...
        fs_create_dir(s(nested.clone())).expect("create dir");
        assert!(nested.is_dir());
        let err = fs_create_dir(s(nested)).unwrap_err();
        assert!(err.contains("already exists"), "got: {err}");
    }
    ...
    fn rename_moves_and_never_overwrites() {
        ...
        fs_rename(s(from.clone()), s(to.clone())).expect("rename");
        assert!(!from.exists());
        assert_eq!(std::fs::read(&to).unwrap(), b"payload");

        // Missing source is reported, not silently ignored.
        let err = fs_rename(s(from), s(dir.path().join("c.txt"))).unwrap_err();
        ...
        let err = fs_rename(s(to.clone()), s(occupied.clone())).unwrap_err();
        assert!(err.contains("already exists"), "got: {err}");
    ...
    fn delete_removes_file_then_dir_recursively() {
        ...
        fs_delete(s(f.clone())).expect("delete file");
        assert!(!f.exists());

        let sub = dir.path().join("sub");
        std::fs::create_dir_all(sub.join("inner")).unwrap();
        std::fs::write(sub.join("inner/y.txt"), b"y").unwrap();
        fs_delete(s(sub.clone())).expect("delete dir");
        assert!(!sub.exists());

        let err = fs_delete(s(dir.path().join("missing"))).unwrap_err();
        assert!(!err.is_empty());
    }
    ...
    fn delete_does_not_follow_symlink_into_target() {
        ...
        fs_delete(s(link.clone())).expect("delete symlink");
